...
export default function App() {
  ...
  function bewertungAnlegen() {
    if (!selKandidat) { setMeldung({ text: "Bitte einen Kandidaten auswahlen.", ok: false }); return }
    const nums = scores.map(Number)
    if (nums.some(s => isNaN(s) || s < 1 || s > 5)) {
      setMeldung({ text: "Alle Bewertungen mussen zwischen 1 und 5 liegen.", ok: false }); return
    }
    ...
    <div className="min-h-screen bg-gradient-to-b from-stone-50 to-white">
      ...
      <header className="bg-white/80 backdrop-blur-sm border-b border-stone-100">
        <div className="max-w-6xl mx-auto px-6 py-5 flex items-center justify-between">
          <div>
            ...
            <p className="text-[13px] text-stone-400 mt-1">Stored Procedures -- interaktiv erklaert</p>
          </div>
      ...
      <div className="bg-violet-50/60 border-b border-violet-100 px-6 py-3">
        <p className="max-w-6xl mx-auto text-[13px] text-violet-700/80">
          <span className="font-semibold">So funktioniert's:</span> Jeder Filter und Button ist eine Stored Procedure -- sie holt, prueft oder berechnet Daten fuer dich im Hintergrund.
        </p>
      </div>
      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-5">
            {/* SP 1 */}
            <SpotlightCard spotlightColor="rgba(139, 92, 246, 0.15)" className="rounded-xl">
                ...
                <CardContent className="space-y-4">
                  <div>
                    ...
                    <Select value={selKandidat} onValueChange={v => v && setSelKandidat(v)}>
                      <SelectTrigger className="h-10"><SelectValue placeholder="Bitte auswahlen ..." /></SelectTrigger>
                      <SelectContent>
